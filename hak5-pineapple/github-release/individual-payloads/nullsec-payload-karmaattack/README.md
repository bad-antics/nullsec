...
Respond to all probe requests for mass interception

## ✨ Features

- Fully compatible with WiFi Pineapple Pager
- NullSec library integration
- Detailed logging to `/root/loot/karmaattack/`
- Clean exit handling
...
mkdir -p /root/payloads/user/nullsec/KarmaAttack
wget -O /root/payloads/user/nullsec/KarmaAttack/payload.sh \
  https://raw.githubusercontent.com/bad-antics/nullsec-payload-karmaattack/main/payload.sh
chmod +x /root/payloads/user/nullsec/KarmaAttack/payload.sh
```
...
### From Pineapple UI
1. **Payloads** → **User** → **nullsec** → **KarmaAttack**
2. Click **Run**

### As Targeted Payload
1. **Recon** → Start scan → Select target
2. **Payloads** → **NullSec-KarmaAttack**
