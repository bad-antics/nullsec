...
Clone legitimate APs to intercept traffic

## ✨ Features

- Fully compatible with WiFi Pineapple Pager
- NullSec library integration
- Detailed logging to `/root/loot/eviltwin/`
- Clean exit handling
...
mkdir -p /root/payloads/user/nullsec/EvilTwin
wget -O /root/payloads/user/nullsec/EvilTwin/payload.sh \
  https://raw.githubusercontent.com/bad-antics/nullsec-payload-eviltwin/main/payload.sh
chmod +x /root/payloads/user/nullsec/EvilTwin/payload.sh
```
...
### From Pineapple UI
1. **Payloads** → **User** → **nullsec** → **EvilTwin**
2. Click **Run**

### As Targeted Payload
1. **Recon** → Start scan → Select target
2. **Payloads** → **NullSec-EvilTwin**
