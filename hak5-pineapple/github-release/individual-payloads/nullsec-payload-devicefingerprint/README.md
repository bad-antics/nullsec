...
Identify and fingerprint wireless devices

## ✨ Features

- Fully compatible with WiFi Pineapple Pager
- NullSec library integration
- Detailed logging to `/root/loot/devicefingerprint/`
- Clean exit handling
...
mkdir -p /root/payloads/user/nullsec/DeviceFingerprint
wget -O /root/payloads/user/nullsec/DeviceFingerprint/payload.sh \
  https://raw.githubusercontent.com/bad-antics/nullsec-payload-devicefingerprint/main/payload.sh
chmod +x /root/payloads/user/nullsec/DeviceFingerprint/payload.sh
```
...
### From Pineapple UI
1. **Payloads** → **User** → **nullsec** → **DeviceFingerprint**
2. Click **Run**

### As Targeted Payload
1. **Recon** → Start scan → Select target
2. **Payloads** → **NullSec-DeviceFingerprint**
