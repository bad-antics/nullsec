...
Audio-based disruption and alert payload

## ✨ Features

- Fully compatible with WiFi Pineapple Pager
- NullSec library integration
- Detailed logging to `/root/loot/banshee/`
- Clean exit handling
...
mkdir -p /root/payloads/user/nullsec/Banshee
wget -O /root/payloads/user/nullsec/Banshee/payload.sh \
  https://raw.githubusercontent.com/bad-antics/nullsec-payload-banshee/main/payload.sh
chmod +x /root/payloads/user/nullsec/Banshee/payload.sh
```
...
### From Pineapple UI
1. **Payloads** → **User** → **nullsec** → **Banshee**
2. Click **Run**

### As Targeted Payload
1. **Recon** → Start scan → Select target
2. **Payloads** → **NullSec-Banshee**
