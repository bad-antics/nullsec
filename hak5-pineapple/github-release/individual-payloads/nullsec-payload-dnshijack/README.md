...
DNS interception and redirection attack

## ✨ Features

- Fully compatible with WiFi Pineapple Pager
- NullSec library integration
- Detailed logging to `/root/loot/dnshijack/`
- Clean exit handling
...
mkdir -p /root/payloads/user/nullsec/DNSHijack
wget -O /root/payloads/user/nullsec/DNSHijack/payload.sh \
  https://raw.githubusercontent.com/bad-antics/nullsec-payload-dnshijack/main/payload.sh
chmod +x /root/payloads/user/nullsec/DNSHijack/payload.sh
```
...
### From Pineapple UI
1. **Payloads** → **User** → **nullsec** → **DNSHijack**
2. Click **Run**

### As Targeted Payload
1. **Recon** → Start scan → Select target
2. **Payloads** → **NullSec-DNSHijack**
