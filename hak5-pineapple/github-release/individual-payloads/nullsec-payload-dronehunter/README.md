...
Detect and identify drone WiFi signatures

## ✨ Features

- Fully compatible with WiFi Pineapple Pager
- NullSec library integration
- Detailed logging to `/root/loot/dronehunter/`
- Clean exit handling
...
mkdir -p /root/payloads/user/nullsec/DroneHunter
wget -O /root/payloads/user/nullsec/DroneHunter/payload.sh \
  https://raw.githubusercontent.com/bad-antics/nullsec-payload-dronehunter/main/payload.sh
chmod +x /root/payloads/user/nullsec/DroneHunter/payload.sh
```
...
### From Pineapple UI
1. **Payloads** → **User** → **nullsec** → **DroneHunter**
2. Click **Run**

### As Targeted Payload
1. **Recon** → Start scan → Select target
2. **Payloads** → **NullSec-DroneHunter**
