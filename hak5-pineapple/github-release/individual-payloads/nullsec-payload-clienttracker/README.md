...
Track and monitor wireless client devices

## ✨ Features

- Fully compatible with WiFi Pineapple Pager
- NullSec library integration
- Detailed logging to `/root/loot/clienttracker/`
- Clean exit handling
...
mkdir -p /root/payloads/user/nullsec/ClientTracker
wget -O /root/payloads/user/nullsec/ClientTracker/payload.sh \
  https://raw.githubusercontent.com/bad-antics/nullsec-payload-clienttracker/main/payload.sh
chmod +x /root/payloads/user/nullsec/ClientTracker/payload.sh
```
...
### From Pineapple UI
1. **Payloads** → **User** → **nullsec** → **ClientTracker**
2. Click **Run**

### As Targeted Payload
1. **Recon** → Start scan → Select target
2. **Payloads** → **NullSec-ClientTracker**
