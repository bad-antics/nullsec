...
Massive deauthentication flood attack

## ✨ Features

- Fully compatible with WiFi Pineapple Pager
- NullSec library integration
- Detailed logging to `/root/loot/deauthstorm/`
- Clean exit handling
...
mkdir -p /root/payloads/user/nullsec/DeauthStorm
wget -O /root/payloads/user/nullsec/DeauthStorm/payload.sh \
  https://raw.githubusercontent.com/bad-antics/nullsec-payload-deauthstorm/main/payload.sh
chmod +x /root/payloads/user/nullsec/DeauthStorm/payload.sh
```
...
### From Pineapple UI
1. **Payloads** → **User** → **nullsec** → **DeauthStorm**
2. Click **Run**

### As Targeted Payload
1. **Recon** → Start scan → Select target
2. **Payloads** → **NullSec-DeauthStorm**
