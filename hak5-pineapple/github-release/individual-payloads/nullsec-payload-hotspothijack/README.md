...
Take over existing hotspot connections

## ✨ Features

- Fully compatible with WiFi Pineapple Pager
- NullSec library integration
- Detailed logging to `/root/loot/hotspothijack/`
- Clean exit handling
...
mkdir -p /root/payloads/user/nullsec/HotspotHijack
wget -O /root/payloads/user/nullsec/HotspotHijack/payload.sh \
  https://raw.githubusercontent.com/bad-antics/nullsec-payload-hotspothijack/main/payload.sh
chmod +x /root/payloads/user/nullsec/HotspotHijack/payload.sh
```
...
### From Pineapple UI
1. **Payloads** → **User** → **nullsec** → **HotspotHijack**
2. Click **Run**

### As Targeted Payload
1. **Recon** → Start scan → Select target
2. **Payloads** → **NullSec-HotspotHijack**
