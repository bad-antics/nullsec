...
Jam specific WiFi channels to disrupt communications

## ✨ Features

- Fully compatible with WiFi Pineapple Pager
- NullSec library integration
- Detailed logging to `/root/loot/channeljammer/`
- Clean exit handling
...
mkdir -p /root/payloads/user/nullsec/ChannelJammer
wget -O /root/payloads/user/nullsec/ChannelJammer/payload.sh \
  https://raw.githubusercontent.com/bad-antics/nullsec-payload-channeljammer/main/payload.sh
chmod +x /root/payloads/user/nullsec/ChannelJammer/payload.sh
```
...
### From Pineapple UI
1. **Payloads** → **User** → **nullsec** → **ChannelJammer**
2. Click **Run**

### As Targeted Payload
1. **Recon** → Start scan → Select target
2. **Payloads** → **NullSec-ChannelJammer**
