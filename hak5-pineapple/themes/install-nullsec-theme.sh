#!/bin/bash
# NullSec Theme Installer for WiFi Pineapple Pager
# Creates a custom NullSec theme based on existing themes

echo "
╔══════════════════════════════════════════════════════════════════╗
║            NullSec Theme Installer for Pineapple Pager           ║
╠══════════════════════════════════════════════════════════════════╣
║  Creating custom NullSec theme with red/black hacker aesthetic   ║
╚══════════════════════════════════════════════════════════════════╝
"

THEME_DIR="/mmc/root/themes/nullsec"
BASE_THEME="/mmc/root/themes/mayhem-red"

echo "[*] Creating NullSec theme directory..."
mkdir -p "$THEME_DIR"

echo "[*] Copying mayhem-red theme as base..."
cp -r "$BASE_THEME"/* "$THEME_DIR/" 2>/dev/null

echo "[*] Creating custom theme.json..."
cat > "$THEME_DIR/theme.json" << 'THEMEJSON'
{
  "author": "NullSec",
  "theme_version": "1.0",
  "theme_framework_version": "0.5",
  ...
  "generic_menus": {
    ...
    "dashboard_path": "components/dashboards/nullsec_dashboard.json",
  ...
  "recon_lists": { "recon_list": "components/recon/recon_results_list.json" },
  ...
  "color_palette":{
    ...
    "red":{ "r":200, "g":0, "b":0 },
    "dark_red":{ "r":100, "g":0, "b":0 },
    "yellow":{ "r":255, "g":200, "b":0 },
    "gray":{ "r":100, "g":100, "b":100 },
    "green":{ "r":0, "g":255, "b":0 },
    "dark_green":{ "r":0, "g":100, "b":0 },
    "disabled_light_gray":{ "r":80, "g":80, "b":80 },
    "disabled_dark_gray":{ "r":40, "g":40, "b":40 },
    "medium_gray":{ "r":60, "g":60, "b":60 },
    "white":{ "r":255, "g":255, "b":255 },
    "orange":{ "r":255, "g":100, "b":0 },
    "dark_orange":{ "r":150, "g":60, "b":0 },
    "light_blue":{ "r":100, "g":200, "b":255 },
    "cyan":{ "r":0, "g":255, "b":200 },
    "dark_cyan":{ "r":0, "g":150, "b":100 },
    "purple":{ "r":150, "g":0, "b":200 },
    "dark_purple":{ "r":80, "g":0, "b":100 },
    "magenta":{ "r":255, "g":0, "b":150 },
    "dark_magenta":{ "r":150, "g":0, "b":80 },
    "light_green":{ "r":100, "g":255, "b":100 },
    "olive":{ "r":100, "g":100, "b":0 },
    "dark_olive":{ "r":50, "g":50, "b":0 },
    "brown":{ "r":100, "g":50, "b":0 },
    "tan":{ "r":180, "g":150, "b":100 },
    "teal":{ "r":0, "g":150, "b":150 },
    "dark_teal":{ "r":0, "g":80, "b":80 },
    "navy":{ "r":0, "g":0, "b":80 },
    "light_yellow":{ "r":255, "g":255, "b":150 },
    "soft_white":{ "r":200, "g":200, "b":200 },
    "nullsec_red":{ "r":200, "g":0, "b":0 },
    "nullsec_dark":{ "r":15, "g":15, "b":15 },
    "nullsec_glow":{ "r":255, "g":50, "b":50 },
    "matrix_green":{ "r":0, "g":255, "b":65 },
    "hacker_green":{ "r":57, "g":255, "b":20 }
  },
  ...
  "_unused" : {
    ...
    "payload_shortcuts": {
      "nullsec_autopwn": "/root/payloads/nullsec/attacks/nullsec-autopwn.sh",
      "nullsec_karma": "/root/payloads/nullsec/attacks/nullsec-karma.sh"
    },
    "simple_images": {}
  }
}
...
echo "[*] Creating custom NullSec dashboard..."
mkdir -p "$THEME_DIR/components/dashboards"

# Check if source dashboard exists and copy/modify
if [ -f "$BASE_THEME/components/dashboards/main_dashboard.json" ]; then
    cp "$BASE_THEME/components/dashboards/main_dashboard.json" "$THEME_DIR/components/dashboards/nullsec_dashboard.json"
    # Modify dashboard title colors
    sed -i 's/"text_color_palette": "white"/"text_color_palette": "nullsec_red"/g' "$THEME_DIR/components/dashboards/nullsec_dashboard.json"
    sed -i 's/"recolor_palette": "blue"/"recolor_palette": "nullsec_glow"/g' "$THEME_DIR/components/dashboards/nullsec_dashboard.json"
elif [ -f "$THEME_DIR/components/dashboards/dashboard_main.json" ]; then
    cp "$THEME_DIR/components/dashboards/dashboard_main.json" "$THEME_DIR/components/dashboards/nullsec_dashboard.json"
fi

echo "[*] Customizing color scheme in component files..."
# Update color references in JSON files
find "$THEME_DIR/components" -name "*.json" -type f 2>/dev/null | while read file; do
    # Make highlights red instead of other colors
    sed -i 's/"recolor_palette": "cyan"/"recolor_palette": "nullsec_red"/g' "$file" 2>/dev/null
    sed -i 's/"recolor_palette": "magenta"/"recolor_palette": "nullsec_glow"/g' "$file" 2>/dev/null
done

echo "[*] Creating README..."
cat > "$THEME_DIR/README" << 'README'
NullSec Theme for WiFi Pineapple Pager
======================================
Custom red/black hacker theme

Color Scheme:
- Primary: Red (#C80000)
- Accent: Bright Red Glow (#FF3232)
...
╠══════════════════════════════════════════════════════════════════╣
║  Theme: NullSec                                                   ║
║  Location: /mmc/root/themes/nullsec                              ║
║                                                                   ║
║  To activate:                                                     ║
║  Settings > Display > Theme > Select 'nullsec'                   ║
║                                                                   ║
