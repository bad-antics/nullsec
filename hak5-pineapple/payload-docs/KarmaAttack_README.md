...
Respond to all probe requests. This payload is part of the NullSec Pineapple Suite - the ultimate payload collection for WiFi Pineapple Pager.

## ✨ Features

- ✅ Universal probe response
- ✅ SSID collection
- ✅ Client tracking
...
```bash
# Download payload
wget https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/KarmaAttack/payload.sh

# Upload to Pager
scp payload.sh root@172.16.42.1:/root/payloads/user/nullsec/KarmaAttack/
```
...
3. Choose **NullSec-KarmaAttack** from payload menu
...
├── karmaattack_YYYYMMDD_HHMMSS.log
└── captures/
```

## 🔧 Configuration

Edit `/root/payloads/library/nullsec-lib.sh` for global settings, or modify the payload script directly for KarmaAttack-specific options.
