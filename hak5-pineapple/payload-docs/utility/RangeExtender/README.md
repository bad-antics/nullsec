...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/RangeExtender && wget -qO /root/payloads/user/nullsec/RangeExtender/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/utility/RangeExtender/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/utility/RangeExtender/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/RangeExtender/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → RangeExtender → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-RangeExtender
```
...
├── rangeextender_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/RangeExtender/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-RangeExtender/payload.sh
/root/payloads/recon/client/NullSec-RangeExtender/payload.sh
```
