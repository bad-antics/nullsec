...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/BootOptimizer && wget -qO /root/payloads/user/nullsec/BootOptimizer/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/utility/BootOptimizer/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/utility/BootOptimizer/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/BootOptimizer/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → BootOptimizer → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-BootOptimizer
```
...
# Run directly
/root/payloads/user/nullsec/BootOptimizer/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/BootOptimizer/
├── bootoptimizer_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/BootOptimizer/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-BootOptimizer/payload.sh
/root/payloads/recon/client/NullSec-BootOptimizer/payload.sh
```
