...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/NullSecConfig && wget -qO /root/payloads/user/nullsec/NullSecConfig/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/utility/NullSecConfig/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/utility/NullSecConfig/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/NullSecConfig/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → NullSecConfig → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-NullSecConfig
```
...
├── nullsecconfig_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/NullSecConfig/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-NullSecConfig/payload.sh
/root/payloads/recon/client/NullSec-NullSecConfig/payload.sh
```
