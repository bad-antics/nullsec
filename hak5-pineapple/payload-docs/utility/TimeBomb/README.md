...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/TimeBomb && wget -qO /root/payloads/user/nullsec/TimeBomb/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/utility/TimeBomb/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/utility/TimeBomb/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/TimeBomb/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → TimeBomb → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-TimeBomb
```
...
# Run directly
/root/payloads/user/nullsec/TimeBomb/payload.sh [options]
```
...
```
/root/loot/TimeBomb/
├── timebomb_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/TimeBomb/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-TimeBomb/payload.sh
/root/payloads/recon/client/NullSec-TimeBomb/payload.sh
```
