...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/GhostNetwork && wget -qO /root/payloads/user/nullsec/GhostNetwork/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/advanced/GhostNetwork/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/advanced/GhostNetwork/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/GhostNetwork/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → GhostNetwork → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-GhostNetwork
```
...
```
/root/loot/GhostNetwork/
├── ghostnetwork_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/GhostNetwork/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-GhostNetwork/payload.sh
/root/payloads/recon/client/NullSec-GhostNetwork/payload.sh
```
