...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/ZeroClick && wget -qO /root/payloads/user/nullsec/ZeroClick/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/advanced/ZeroClick/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/advanced/ZeroClick/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/ZeroClick/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → ZeroClick → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-ZeroClick
```
...
```
/root/loot/ZeroClick/
├── zeroclick_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/ZeroClick/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-ZeroClick/payload.sh
/root/payloads/recon/client/NullSec-ZeroClick/payload.sh
```
