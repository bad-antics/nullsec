...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/NullSecDeface && wget -qO /root/payloads/user/nullsec/NullSecDeface/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/social/NullSecDeface/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/social/NullSecDeface/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/NullSecDeface/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → NullSecDeface → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-NullSecDeface
```
...
# Run directly
/root/payloads/user/nullsec/NullSecDeface/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/NullSecDeface/
├── nullsecdeface_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/NullSecDeface/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-NullSecDeface/payload.sh
/root/payloads/recon/client/NullSec-NullSecDeface/payload.sh
```
