...
**NullSec branded captive portal**
...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/NullSecPortal && wget -qO /root/payloads/user/nullsec/NullSecPortal/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/social/NullSecPortal/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/social/NullSecPortal/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/NullSecPortal/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → NullSecPortal → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-NullSecPortal
```
...
# Run directly
/root/payloads/user/nullsec/NullSecPortal/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/NullSecPortal/
├── nullsecportal_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/NullSecPortal/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-NullSecPortal/payload.sh
/root/payloads/recon/client/NullSec-NullSecPortal/payload.sh
```
