...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/Honeypot && wget -qO /root/payloads/user/nullsec/Honeypot/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/social/Honeypot/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/social/Honeypot/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/Honeypot/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → Honeypot → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-Honeypot
```
...
```
/root/loot/Honeypot/
├── honeypot_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/Honeypot/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-Honeypot/payload.sh
/root/payloads/recon/client/NullSec-Honeypot/payload.sh
```
