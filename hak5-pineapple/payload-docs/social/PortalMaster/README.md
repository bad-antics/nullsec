...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/PortalMaster && wget -qO /root/payloads/user/nullsec/PortalMaster/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/social/PortalMaster/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/social/PortalMaster/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/PortalMaster/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → PortalMaster → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-PortalMaster
```
...
├── portalmaster_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/PortalMaster/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-PortalMaster/payload.sh
/root/payloads/recon/client/NullSec-PortalMaster/payload.sh
```
