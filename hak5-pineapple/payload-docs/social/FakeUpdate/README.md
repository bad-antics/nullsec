...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/FakeUpdate && wget -qO /root/payloads/user/nullsec/FakeUpdate/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/social/FakeUpdate/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/social/FakeUpdate/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/FakeUpdate/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → FakeUpdate → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-FakeUpdate
```
...
```
/root/loot/FakeUpdate/
├── fakeupdate_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/FakeUpdate/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-FakeUpdate/payload.sh
/root/payloads/recon/client/NullSec-FakeUpdate/payload.sh
```
