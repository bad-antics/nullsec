...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/SSIDPranks && wget -qO /root/payloads/user/nullsec/SSIDPranks/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/pranks/SSIDPranks/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/pranks/SSIDPranks/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/SSIDPranks/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → SSIDPranks → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-SSIDPranks
```
...
# Run directly
/root/payloads/user/nullsec/SSIDPranks/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/SSIDPranks/
├── ssidpranks_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/SSIDPranks/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-SSIDPranks/payload.sh
/root/payloads/recon/client/NullSec-SSIDPranks/payload.sh
```
