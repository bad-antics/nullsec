...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/BeaconSpam && wget -qO /root/payloads/user/nullsec/BeaconSpam/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/pranks/BeaconSpam/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/pranks/BeaconSpam/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/BeaconSpam/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → BeaconSpam → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-BeaconSpam
```
...
```
/root/loot/BeaconSpam/
├── beaconspam_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/BeaconSpam/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-BeaconSpam/payload.sh
/root/payloads/recon/client/NullSec-BeaconSpam/payload.sh
```
