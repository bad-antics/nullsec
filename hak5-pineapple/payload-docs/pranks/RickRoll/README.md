...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/RickRoll && wget -qO /root/payloads/user/nullsec/RickRoll/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/pranks/RickRoll/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/pranks/RickRoll/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/RickRoll/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → RickRoll → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-RickRoll
```
...
# Run directly
/root/payloads/user/nullsec/RickRoll/payload.sh [options]
```
...
```
/root/loot/RickRoll/
├── rickroll_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/RickRoll/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-RickRoll/payload.sh
/root/payloads/recon/client/NullSec-RickRoll/payload.sh
```
