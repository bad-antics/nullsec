...
Flood the air with fake SSIDs. This payload is part of the NullSec Pineapple Suite - the ultimate payload collection for WiFi Pineapple Pager.
...
```bash
# Download payload
wget https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/pranks/BeaconSpam/payload.sh

# Upload to Pager
scp payload.sh root@172.16.42.1:/root/payloads/user/nullsec/BeaconSpam/
```
...
3. Choose **NullSec-BeaconSpam** from payload menu
...
├── beaconspam_YYYYMMDD_HHMMSS.log
└── captures/
```

## 🔧 Configuration

Edit `/root/payloads/library/nullsec-lib.sh` for global settings, or modify the payload script directly for BeaconSpam-specific options.
