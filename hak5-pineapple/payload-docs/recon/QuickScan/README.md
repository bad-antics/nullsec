...
**Fast network discovery and enumeration**
...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/QuickScan && wget -qO /root/payloads/user/nullsec/QuickScan/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/QuickScan/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/QuickScan/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/QuickScan/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → QuickScan → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-QuickScan
```
...
# Run directly
/root/payloads/user/nullsec/QuickScan/payload.sh [options]
```
...
```
/root/loot/QuickScan/
├── quickscan_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/QuickScan/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-QuickScan/payload.sh
/root/payloads/recon/client/NullSec-QuickScan/payload.sh
```
