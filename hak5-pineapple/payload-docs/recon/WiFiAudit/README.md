...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/WiFiAudit && wget -qO /root/payloads/user/nullsec/WiFiAudit/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/WiFiAudit/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/WiFiAudit/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/WiFiAudit/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → WiFiAudit → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-WiFiAudit
```
...
```
/root/loot/WiFiAudit/
├── wifiaudit_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/WiFiAudit/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-WiFiAudit/payload.sh
/root/payloads/recon/client/NullSec-WiFiAudit/payload.sh
```
