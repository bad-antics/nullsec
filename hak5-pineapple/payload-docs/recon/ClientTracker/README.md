...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/ClientTracker && wget -qO /root/payloads/user/nullsec/ClientTracker/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/ClientTracker/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/ClientTracker/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/ClientTracker/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → ClientTracker → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-ClientTracker
```
...
# Run directly
/root/payloads/user/nullsec/ClientTracker/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/ClientTracker/
├── clienttracker_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/ClientTracker/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-ClientTracker/payload.sh
/root/payloads/recon/client/NullSec-ClientTracker/payload.sh
```
