...
**Passive reconnaissance without probing**
...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/StealthRecon && wget -qO /root/payloads/user/nullsec/StealthRecon/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/StealthRecon/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/StealthRecon/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/StealthRecon/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → StealthRecon → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-StealthRecon
```
...
# Run directly
/root/payloads/user/nullsec/StealthRecon/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/StealthRecon/
├── stealthrecon_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/StealthRecon/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-StealthRecon/payload.sh
/root/payloads/recon/client/NullSec-StealthRecon/payload.sh
```
