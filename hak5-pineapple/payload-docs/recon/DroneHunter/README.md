...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/DroneHunter && wget -qO /root/payloads/user/nullsec/DroneHunter/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/DroneHunter/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/DroneHunter/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/DroneHunter/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → DroneHunter → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-DroneHunter
```
...
├── dronehunter_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/DroneHunter/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-DroneHunter/payload.sh
/root/payloads/recon/client/NullSec-DroneHunter/payload.sh
```
