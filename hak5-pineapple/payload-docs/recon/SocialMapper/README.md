...
**Map social connections via probes**
...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/SocialMapper && wget -qO /root/payloads/user/nullsec/SocialMapper/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/SocialMapper/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/SocialMapper/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/SocialMapper/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → SocialMapper → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-SocialMapper
```
...
# Run directly
/root/payloads/user/nullsec/SocialMapper/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/SocialMapper/
├── socialmapper_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/SocialMapper/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-SocialMapper/payload.sh
/root/payloads/recon/client/NullSec-SocialMapper/payload.sh
```
