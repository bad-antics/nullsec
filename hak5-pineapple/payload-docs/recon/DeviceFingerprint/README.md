...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/DeviceFingerprint && wget -qO /root/payloads/user/nullsec/DeviceFingerprint/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/DeviceFingerprint/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/DeviceFingerprint/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/DeviceFingerprint/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → DeviceFingerprint → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-DeviceFingerprint
```
...
├── devicefingerprint_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/DeviceFingerprint/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-DeviceFingerprint/payload.sh
/root/payloads/recon/client/NullSec-DeviceFingerprint/payload.sh
```
