...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/NetworkMapper && wget -qO /root/payloads/user/nullsec/NetworkMapper/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/NetworkMapper/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/NetworkMapper/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/NetworkMapper/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → NetworkMapper → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-NetworkMapper
```
...
# Run directly
/root/payloads/user/nullsec/NetworkMapper/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/NetworkMapper/
├── networkmapper_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/NetworkMapper/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-NetworkMapper/payload.sh
/root/payloads/recon/client/NullSec-NetworkMapper/payload.sh
```
