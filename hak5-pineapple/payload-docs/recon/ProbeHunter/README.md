...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/ProbeHunter && wget -qO /root/payloads/user/nullsec/ProbeHunter/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/ProbeHunter/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/ProbeHunter/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/ProbeHunter/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → ProbeHunter → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-ProbeHunter
```
...
# Run directly
/root/payloads/user/nullsec/ProbeHunter/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/ProbeHunter/
├── probehunter_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/ProbeHunter/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-ProbeHunter/payload.sh
/root/payloads/recon/client/NullSec-ProbeHunter/payload.sh
```
