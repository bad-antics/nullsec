...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/VendorHunt && wget -qO /root/payloads/user/nullsec/VendorHunt/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/VendorHunt/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/VendorHunt/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/VendorHunt/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → VendorHunt → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-VendorHunt
```
...
# Run directly
/root/payloads/user/nullsec/VendorHunt/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/VendorHunt/
├── vendorhunt_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/VendorHunt/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-VendorHunt/payload.sh
/root/payloads/recon/client/NullSec-VendorHunt/payload.sh
```
