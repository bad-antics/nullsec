...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/IoTScanner && wget -qO /root/payloads/user/nullsec/IoTScanner/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/recon/IoTScanner/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/recon/IoTScanner/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/IoTScanner/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → IoTScanner → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-IoTScanner
```
...
# Run directly
/root/payloads/user/nullsec/IoTScanner/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/IoTScanner/
├── iotscanner_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/IoTScanner/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-IoTScanner/payload.sh
/root/payloads/recon/client/NullSec-IoTScanner/payload.sh
```
