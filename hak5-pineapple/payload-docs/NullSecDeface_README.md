...
Hacker-style deface portal. This payload is part of the NullSec Pineapple Suite - the ultimate payload collection for WiFi Pineapple Pager.
...
```bash
# Download payload
wget https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/social/NullSecDeface/payload.sh

# Upload to Pager
scp payload.sh root@172.16.42.1:/root/payloads/user/nullsec/NullSecDeface/
```
...
3. Choose **NullSec-NullSecDeface** from payload menu
...
├── nullsecdeface_YYYYMMDD_HHMMSS.log
└── captures/
```

## 🔧 Configuration

Edit `/root/payloads/library/nullsec-lib.sh` for global settings, or modify the payload script directly for NullSecDeface-specific options.
