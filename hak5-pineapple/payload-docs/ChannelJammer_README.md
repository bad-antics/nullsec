...
Channel-specific disruption. This payload is part of the NullSec Pineapple Suite - the ultimate payload collection for WiFi Pineapple Pager.
...
```bash
# Download payload
wget https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/ChannelJammer/payload.sh

# Upload to Pager
scp payload.sh root@172.16.42.1:/root/payloads/user/nullsec/ChannelJammer/
```
...
3. Choose **NullSec-ChannelJammer** from payload menu
...
├── channeljammer_YYYYMMDD_HHMMSS.log
└── captures/
```

## 🔧 Configuration

Edit `/root/payloads/library/nullsec-lib.sh` for global settings, or modify the payload script directly for ChannelJammer-specific options.
