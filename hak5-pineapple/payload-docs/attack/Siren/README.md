...
**Alert and notification attacks**
...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/Siren && wget -qO /root/payloads/user/nullsec/Siren/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/Siren/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/Siren/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/Siren/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → Siren → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-Siren
```
...
# Run directly
/root/payloads/user/nullsec/Siren/payload.sh [options]
```
...
```
/root/loot/Siren/
├── siren_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/Siren/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-Siren/payload.sh
/root/payloads/recon/client/NullSec-Siren/payload.sh
```
