...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/Specter && wget -qO /root/payloads/user/nullsec/Specter/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/Specter/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/Specter/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/Specter/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → Specter → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-Specter
```
...
# Run directly
/root/payloads/user/nullsec/Specter/payload.sh [options]
```
...
```
/root/loot/Specter/
├── specter_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/Specter/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-Specter/payload.sh
/root/payloads/recon/client/NullSec-Specter/payload.sh
```
