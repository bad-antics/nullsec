...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/ChannelJammer && wget -qO /root/payloads/user/nullsec/ChannelJammer/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/ChannelJammer/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/ChannelJammer/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/ChannelJammer/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → ChannelJammer → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-ChannelJammer
```
...
# Run directly
/root/payloads/user/nullsec/ChannelJammer/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/ChannelJammer/
├── channeljammer_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/ChannelJammer/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-ChannelJammer/payload.sh
/root/payloads/recon/client/NullSec-ChannelJammer/payload.sh
```
