...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/Banshee && wget -qO /root/payloads/user/nullsec/Banshee/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/Banshee/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/Banshee/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/Banshee/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → Banshee → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-Banshee
```
...
# Run directly
/root/payloads/user/nullsec/Banshee/payload.sh [options]
```
...
```
/root/loot/Banshee/
├── banshee_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/Banshee/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-Banshee/payload.sh
/root/payloads/recon/client/NullSec-Banshee/payload.sh
```
