...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/DeauthStorm && wget -qO /root/payloads/user/nullsec/DeauthStorm/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/DeauthStorm/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/DeauthStorm/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/DeauthStorm/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → DeauthStorm → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-DeauthStorm
```
...
# Run directly
/root/payloads/user/nullsec/DeauthStorm/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/DeauthStorm/
├── deauthstorm_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/DeauthStorm/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-DeauthStorm/payload.sh
/root/payloads/recon/client/NullSec-DeauthStorm/payload.sh
```
