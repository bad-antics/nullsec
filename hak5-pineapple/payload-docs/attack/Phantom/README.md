...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/Phantom && wget -qO /root/payloads/user/nullsec/Phantom/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/Phantom/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/Phantom/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/Phantom/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → Phantom → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-Phantom
```
...
```
/root/loot/Phantom/
├── phantom_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/Phantom/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-Phantom/payload.sh
/root/payloads/recon/client/NullSec-Phantom/payload.sh
```
