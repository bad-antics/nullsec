...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/MassDeauth && wget -qO /root/payloads/user/nullsec/MassDeauth/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/MassDeauth/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/MassDeauth/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/MassDeauth/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → MassDeauth → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-MassDeauth
```
...
# Run directly
/root/payloads/user/nullsec/MassDeauth/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/MassDeauth/
├── massdeauth_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/MassDeauth/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-MassDeauth/payload.sh
/root/payloads/recon/client/NullSec-MassDeauth/payload.sh
```
