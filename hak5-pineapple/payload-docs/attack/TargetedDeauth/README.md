# NullSec Payload: TargetedDeauth
...
**Precision deauth against specific clients**
...
- ✅ Single target focus
- ✅ Persistent reconnect detection
...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/TargetedDeauth && wget -qO /root/payloads/user/nullsec/TargetedDeauth/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/TargetedDeauth/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/TargetedDeauth/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/TargetedDeauth/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → TargetedDeauth → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-TargetedDeauth
```
...
# Run directly
/root/payloads/user/nullsec/TargetedDeauth/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/TargetedDeauth/
├── targeteddeauth_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/TargetedDeauth/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-TargetedDeauth/payload.sh
/root/payloads/recon/client/NullSec-TargetedDeauth/payload.sh
```
