...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/Reaper && wget -qO /root/payloads/user/nullsec/Reaper/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/Reaper/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/Reaper/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/Reaper/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → Reaper → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-Reaper
```
...
# Run directly
/root/payloads/user/nullsec/Reaper/payload.sh [options]
```
...
```
/root/loot/Reaper/
├── reaper_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/Reaper/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-Reaper/payload.sh
/root/payloads/recon/client/NullSec-Reaper/payload.sh
```
