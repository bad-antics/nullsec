...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/AuthFlood && wget -qO /root/payloads/user/nullsec/AuthFlood/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/AuthFlood/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/AuthFlood/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/AuthFlood/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → AuthFlood → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-AuthFlood
```
...
```
/root/loot/AuthFlood/
├── authflood_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/AuthFlood/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-AuthFlood/payload.sh
/root/payloads/recon/client/NullSec-AuthFlood/payload.sh
```
