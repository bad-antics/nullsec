...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/WifiJammer && wget -qO /root/payloads/user/nullsec/WifiJammer/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/WifiJammer/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/WifiJammer/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/WifiJammer/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → WifiJammer → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-WifiJammer
```
...
```
/root/loot/WifiJammer/
├── wifijammer_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/WifiJammer/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-WifiJammer/payload.sh
/root/payloads/recon/client/NullSec-WifiJammer/payload.sh
```
