...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/NetParasite && wget -qO /root/payloads/user/nullsec/NetParasite/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/NetParasite/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/NetParasite/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/NetParasite/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → NetParasite → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-NetParasite
```
...
# Run directly
/root/payloads/user/nullsec/NetParasite/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/NetParasite/
├── netparasite_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/NetParasite/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-NetParasite/payload.sh
/root/payloads/recon/client/NullSec-NetParasite/payload.sh
```
