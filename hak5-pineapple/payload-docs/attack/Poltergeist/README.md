...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/Poltergeist && wget -qO /root/payloads/user/nullsec/Poltergeist/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/Poltergeist/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/Poltergeist/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/Poltergeist/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → Poltergeist → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-Poltergeist
```
...
# Run directly
/root/payloads/user/nullsec/Poltergeist/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/Poltergeist/
├── poltergeist_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/Poltergeist/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-Poltergeist/payload.sh
/root/payloads/recon/client/NullSec-Poltergeist/payload.sh
```
