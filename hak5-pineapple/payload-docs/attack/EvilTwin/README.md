...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/EvilTwin && wget -qO /root/payloads/user/nullsec/EvilTwin/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/EvilTwin/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/EvilTwin/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/EvilTwin/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → EvilTwin → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-EvilTwin
```
...
```
/root/loot/EvilTwin/
├── eviltwin_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/EvilTwin/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-EvilTwin/payload.sh
/root/payloads/recon/client/NullSec-EvilTwin/payload.sh
```
