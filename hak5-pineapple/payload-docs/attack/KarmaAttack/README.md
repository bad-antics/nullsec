...
- ✅ Capture wandering clients
- ✅ SSID history collection
- ✅ Auto-portal deployment
...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/KarmaAttack && wget -qO /root/payloads/user/nullsec/KarmaAttack/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/KarmaAttack/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/KarmaAttack/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/KarmaAttack/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → KarmaAttack → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-KarmaAttack
```
...
# Run directly
/root/payloads/user/nullsec/KarmaAttack/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/KarmaAttack/
├── karmaattack_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/KarmaAttack/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-KarmaAttack/payload.sh
/root/payloads/recon/client/NullSec-KarmaAttack/payload.sh
```
