...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/Wraith && wget -qO /root/payloads/user/nullsec/Wraith/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/Wraith/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/Wraith/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/Wraith/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → Wraith → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-Wraith
```
...
# Run directly
/root/payloads/user/nullsec/Wraith/payload.sh [options]
```
...
```
/root/loot/Wraith/
├── wraith_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/Wraith/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-Wraith/payload.sh
/root/payloads/recon/client/NullSec-Wraith/payload.sh
```
