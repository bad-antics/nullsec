...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/DNSHijack && wget -qO /root/payloads/user/nullsec/DNSHijack/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/DNSHijack/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/DNSHijack/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/DNSHijack/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → DNSHijack → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-DNSHijack
```
...
# Run directly
/root/payloads/user/nullsec/DNSHijack/payload.sh [options]
```
...
```
/root/loot/DNSHijack/
├── dnshijack_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/DNSHijack/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-DNSHijack/payload.sh
/root/payloads/recon/client/NullSec-DNSHijack/payload.sh
```
