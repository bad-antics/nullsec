...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/HotspotHijack && wget -qO /root/payloads/user/nullsec/HotspotHijack/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/attack/HotspotHijack/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/attack/HotspotHijack/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/HotspotHijack/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → HotspotHijack → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-HotspotHijack
```
...
# Run directly
/root/payloads/user/nullsec/HotspotHijack/payload.sh [options]
```

## 📁 Output Location

```
/root/loot/HotspotHijack/
├── hotspothijack_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/HotspotHijack/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-HotspotHijack/payload.sh
/root/payloads/recon/client/NullSec-HotspotHijack/payload.sh
```
