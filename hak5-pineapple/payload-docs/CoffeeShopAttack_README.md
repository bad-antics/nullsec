...
Public WiFi credential harvesting. This payload is part of the NullSec Pineapple Suite - the ultimate payload collection for WiFi Pineapple Pager.
...
```bash
# Download payload
wget https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/social/CoffeeShopAttack/payload.sh

# Upload to Pager
scp payload.sh root@172.16.42.1:/root/payloads/user/nullsec/CoffeeShopAttack/
```
...
3. Choose **NullSec-CoffeeShopAttack** from payload menu
...
├── coffeeshopattack_YYYYMMDD_HHMMSS.log
└── captures/
```

## 🔧 Configuration

Edit `/root/payloads/library/nullsec-lib.sh` for global settings, or modify the payload script directly for CoffeeShopAttack-specific options.
