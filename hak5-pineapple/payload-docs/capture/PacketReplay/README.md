...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/PacketReplay && wget -qO /root/payloads/user/nullsec/PacketReplay/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/capture/PacketReplay/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/capture/PacketReplay/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/PacketReplay/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → PacketReplay → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-PacketReplay
```
...
├── packetreplay_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/PacketReplay/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-PacketReplay/payload.sh
/root/payloads/recon/client/NullSec-PacketReplay/payload.sh
```
