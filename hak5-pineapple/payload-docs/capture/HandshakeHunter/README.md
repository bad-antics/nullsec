...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Category-Capture-9b59b6?style=for-the-badge" alt="Category">
...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/HandshakeHunter && wget -qO /root/payloads/user/nullsec/HandshakeHunter/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/capture/HandshakeHunter/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/capture/HandshakeHunter/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/HandshakeHunter/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → HandshakeHunter → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-HandshakeHunter
```
...
├── handshakehunter_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/HandshakeHunter/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-HandshakeHunter/payload.sh
/root/payloads/recon/client/NullSec-HandshakeHunter/payload.sh
```
