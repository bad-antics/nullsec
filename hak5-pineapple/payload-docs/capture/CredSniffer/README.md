...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/CredSniffer && wget -qO /root/payloads/user/nullsec/CredSniffer/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/capture/CredSniffer/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/capture/CredSniffer/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/CredSniffer/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → CredSniffer → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-CredSniffer
```
...
├── credsniffer_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/CredSniffer/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-CredSniffer/payload.sh
/root/payloads/recon/client/NullSec-CredSniffer/payload.sh
```
