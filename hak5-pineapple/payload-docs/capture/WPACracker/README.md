...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/WPACracker && wget -qO /root/payloads/user/nullsec/WPACracker/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/capture/WPACracker/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/capture/WPACracker/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/WPACracker/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → WPACracker → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-WPACracker
```
...
├── wpacracker_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/WPACracker/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-WPACracker/payload.sh
/root/payloads/recon/client/NullSec-WPACracker/payload.sh
```
