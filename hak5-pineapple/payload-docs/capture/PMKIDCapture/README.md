...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/PMKIDCapture && wget -qO /root/payloads/user/nullsec/PMKIDCapture/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/capture/PMKIDCapture/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/capture/PMKIDCapture/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/PMKIDCapture/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → PMKIDCapture → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-PMKIDCapture
```
...
├── pmkidcapture_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/PMKIDCapture/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-PMKIDCapture/payload.sh
/root/payloads/recon/client/NullSec-PMKIDCapture/payload.sh
```
