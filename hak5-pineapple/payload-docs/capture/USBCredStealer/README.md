...
### One-Line Install
```bash
ssh root@172.16.52.1 'mkdir -p /root/payloads/user/nullsec/USBCredStealer && wget -qO /root/payloads/user/nullsec/USBCredStealer/payload.sh https://raw.githubusercontent.com/bad-antics/nullsec-pineapple-suite/main/payloads/capture/USBCredStealer/payload.sh'
```
...
# Copy to device
scp nullsec-pineapple-suite/payloads/capture/USBCredStealer/payload.sh \
    root@172.16.52.1:/root/payloads/user/nullsec/USBCredStealer/
```
...
### Standard Execution
```
Dashboard → Payloads → User → nullsec → USBCredStealer → Run
```

### Targeted Execution (from Recon)
```
Dashboard → Recon → [Select Target] → Payloads → NullSec-USBCredStealer
```
...
├── usbcredstealer_YYYYMMDD_HHMMSS.log    # Execution log
...
```bash
vim /root/payloads/user/nullsec/USBCredStealer/payload.sh
```
...
```
/root/payloads/recon/access_point/NullSec-USBCredStealer/payload.sh
/root/payloads/recon/client/NullSec-USBCredStealer/payload.sh
```
